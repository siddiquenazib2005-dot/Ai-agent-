...
import { listProfiles, saveProfile, forgetProfile, testProfile, registerStoredSecrets, PROVIDER_NAMES } from '../settingsStore.js';
...
const SENSITIVE = /(^|\/)(\.env(?:\..*)?|providers\.json|credentials[^/]*|[^/]*\.(key|pem|jks|keystore))$/i;
function repoRoot(value) { const root = fs.realpathSync(String(value)); if (!fs.statSync(root).isDirectory()) throw new Error('Repository unavailable'); return root; }
function fileInfo(root, dir = '') {
  const target = safeResolve(root, dir || '.'); assertInsideRepo(root, target);
  if (!fs.statSync(target).isDirectory()) throw new Error('Folder unavailable');
  return fs.readdirSync(target, { withFileTypes: true }).filter(e => !['.git', 'node_modules'].includes(e.name)).map(e => {
    const rel = path.posix.join(String(dir).replace(/\\/g, '/'), e.name); const st = fs.statSync(path.join(target, e.name));
    return { name: e.name, path: rel, type: e.isDirectory() ? 'folder' : 'file', size: e.isFile() ? st.size : null, modifiedAt: st.mtime.toISOString() };
...
export async function startBridge({ port = 8787, host = '127.0.0.1', token = process.env.BRIDGE_TOKEN || '' } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      // Saved provider profiles WITHOUT key material (see settingsStore.js).
      if (req.method === 'GET' && url.pathname === '/settings') return send(200, { profiles: listProfiles(), providerNames: PROVIDER_NAMES, storage: 'providers.json on the bridge host (chmod 600, gitignored)' });
      if (req.method === 'GET' && url.pathname === '/sessions') return send(200, { sessions: listSessions() });
      if (req.method === 'GET' && url.pathname === '/repositories') {
        const repos = [...new Set([activeRun?.repo, lastRun?.repo, ...listSessions().map(s => s.repo)].filter(Boolean))].filter(p => { try { return fs.statSync(repoRoot(p)).isDirectory(); } catch { return false; } });
        return send(200, { repositories: repos.map(p => ({ path: repoRoot(p), name: path.basename(p) || p })) });
      }
      if (req.method === 'GET' && url.pathname === '/files') {
        const root = repoRoot(url.searchParams.get('repo')); const dir = url.searchParams.get('dir') || '';
        return send(200, { repo: root, dir, entries: fileInfo(root, dir) });
      }
      if (req.method === 'GET' && url.pathname === '/file') {
        const root = repoRoot(url.searchParams.get('repo')); const rel = url.searchParams.get('path') || '';
        if (SENSITIVE.test(rel)) return send(403, { error: 'Sensitive file preview blocked' });
        const file = safeResolve(root, rel); assertInsideRepo(root, file); const stat = fs.statSync(file);
        if (!stat.isFile() || stat.size > 262144) return send(413, { error: 'Preview supports text files up to 256 KB' });
        const bytes = fs.readFileSync(file); if (bytes.includes(0)) return send(415, { error: 'Binary preview unsupported' });
        return send(200, { path: rel, content: bytes.toString('utf8'), size: stat.size });
      }
      ...
      if (url.pathname === '/run') {
        ...
        const { repo, task, mode = 'confirm', plan = false, split = false, providerId, model, resumeSessionId, allowTerminal = true } = json;
        if (typeof repo !== 'string' || typeof task !== 'string' || !repo.trim() || !task.trim()) return send(400, { error: 'Repository and task are required' });
        if (!['confirm', 'auto'].includes(mode) || typeof plan !== 'boolean' || typeof split !== 'boolean' || typeof allowTerminal !== 'boolean') return send(400, { error: 'Invalid execution flags' });
        ...
        running = (async () => {
          try { await runAgentLoop(root, task, { mode, plan, split, providerId: c.providerId, modelOverride: typeof model === 'string' && model ? model : null, resumeSessionId: typeof resumeSessionId === 'string' ? resumeSessionId : null, emitter, confirmAction, signal: controller.signal, cancelRequested: () => cancelled, allowTerminal }); }
          catch (err) {
      ...
      if (url.pathname === '/file') {
        ...
        if (SENSITIVE.test(json.path)) return send(403, { error: 'Sensitive preview blocked' });
      ...
      // Save/update a provider profile. Keys are stored host-side only and are
      // never echoed back, logged, or committed.
      if (url.pathname === '/settings') {
        try { return send(200, { ...saveProfile(json), providerNames: PROVIDER_NAMES }); }
        catch (error) { return send(400, { error: redactSecrets(String(error?.message || 'Could not save provider settings')) }); }
      }
      if (url.pathname === '/settings/forget') {
        if (typeof json.id !== 'string' || !json.id) return send(400, { error: 'Profile id is required' });
        try { return send(200, forgetProfile(json.id)); }
        catch (error) { return send(400, { error: redactSecrets(String(error?.message || 'Could not remove profile')) }); }
      }
      // Real provider round-trip so "Test key" can never report a false success.
      if (url.pathname === '/settings/test') {
        if (activeRun) return send(409, { error: 'Finish the active run before testing a key' });
        return send(200, await testProfile(typeof json.id === 'string' && json.id ? json.id : null, process.env));
      }
