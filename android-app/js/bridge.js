...
(function (global) {
  function createAgentClient(baseUrl, { token = '', fetchImpl = global.fetch.bind(global) } = {}) {
    ...
    const api = { baseUrl, isConnected: () => connected,
      ...
      health: () => request('/health').then(r => r.data), providers: () => request('/providers').then(r => r.data), settings: () => request('/settings').then(r => r.data), saveSettings: body => request('/settings', body).then(r => r.data), testSettings: id => request('/settings/test', id ? { id } : {}).then(r => r.data), forgetSettings: id => request('/settings/forget', { id }).then(r => r.data), listSessions: () => request('/sessions').then(r => r.data), repositories: () => request('/repositories').then(r => r.data),
